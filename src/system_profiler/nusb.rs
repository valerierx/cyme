//! Uses nusb (Rust native) to get system USB information, most of which has parity with lsusb. Uses [`crate::system_profiler`] types to hold data so that it is cross-compatible with macOS system_profiler command.
//!
...
fn get_product_string(
    device_desc: &nusb::descriptors::Descriptor,
...
fn get_configuration_string(
    config_desc: &nusb::descriptors::Configuration,
    handle: &mut Option<UsbDevice>,
) -> Option<String> {
    handle.as_mut().and_then(|h| {
        match h
...
fn get_interface_string(
    interface_desc: &nusb::descriptors::InterfaceGroup,
    handle: &mut Option<UsbDevice>,
) -> Option<String> {
    handle.as_mut().and_then(|h| {
        match h
...
fn build_descriptor_extra(
    handle: &mut Option<UsbDevice>,
    interface_desc: Option<&nusb::Device>,
    extra_bytes: &[u8],
...
fn build_endpoint_descriptor_extra(
    handle: &mut Option<UsbDevice>,
    interface_desc: &nusb::descriptors::InterfaceGroup,
    endpoint_desc: &nusb::descriptors::Endpoint,
...
pub fn build_spdevice(
    device: &nusb::DeviceInfo,
    with_extra: bool,
) -> error::Result<system_profiler::USBDevice> {
    let timeout = Duration::from_secs(1);
    let speed = match usb::Speed::from(device.speed().unwrap_or(nusb::Speed::High)) {
    ...
    let mut usb_device = {
        match device.open() {
            Ok(h) => match h.get_string_descriptor_supported_languages(timeout) {
                Ok(l) => {
                    if l.count() > 0 {
                        Some(UsbDevice {
                            handle: h,
                            language: l.into_iter().next().unwrap(),
                            timeout,
                        })
                    } else {
                        None
